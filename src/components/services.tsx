...
import { Building2, Clock, Hammer, Users } from "lucide-react";
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {services.map((service, id) => (
                        <motion.div
                            key={id}
                            className="group p-6 bg-white hover:bg-[#ffc13b] hover:transition-colors hover:duration-500 border-[2px] shadow-md hover:-translate-y-2 hover:shadow-lg transition-transform cursor-pointer rounded-md"
                            initial={{ opacity: 0, y: 50 }}
                            whileInView={{ opacity: 1, y: 0 }}
                            transition={{
                                duration: 1,
